...
class _ProductModelState extends State<ProductModel> {
  ...
  void _handleDeletePurchase() async {
    ...
    if (response['success']) {
      Helpers.showAlert(context,
          title: 'You have successfully removed this purchase',
          hasAction: true, onPressed: () {
        Navigator.pop(context);
  ...
  Future<void> getStore() async {
    ...
    var stores =
        (response['data'] as List).map((i) => Datum.fromJson(i)).toList();

    if (purchase!.storeName != null) {
      var storeName =
          stores.where((element) => element.storeId == purchase!.storeName);
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (purchase?.extendedEwarrantyCost == '0' ||
                          purchase?.statusCode == '2' ||
                          purchase?.warranty!.substring(0, 1) == 'E' ||
                          purchase!.warranty! == 'W00')
                        GradientButton(
                          hideShadow: true,
                          height: 40,
                          child: Text(
                          ...
                          onPressed: () {
                            print(purchase?.extendedEwarrantyCost);
                            print(purchase?.statusCode);
                            print(purchase?.warranty);
                          },
                        )
                      else
                        RoundButton(
